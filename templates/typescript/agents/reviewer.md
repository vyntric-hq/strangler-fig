...
**Conventions.** Walk `CONVENTIONS.md` section by section against the new code. Business logic in a route handler, SQL outside a store module, an untyped shape crossing a boundary, a missing zod schema on a body-accepting handler, an unbounded list query, a client component importing server-only code. Cite the section you are applying for each finding.

**Completeness of the switch.** Confirm the callers point at the new implementation and the old implementation is actually deleted. Search for lingering imports of the old modules. Dead code left behind is a finding.
